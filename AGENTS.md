...
Golem JournAI is a distributed log analysis and monitoring system built on Golem Cloud. It collects, analyzes, and
visualizes system logs using AI-powered insights, with components compiled to WebAssembly for scalable, intelligent log
processing.
...
- **Collector** (`journai:collector`) - Collects and stores journal entries from monitored hosts
- **Analyzer** (`journai:analyzer`) - Performs AI-powered analysis of log patterns and anomalies
- **Visualizer** (`journai:visualizer`) - Provides web-based dashboards and visualization interfaces

All components share a common library (`common-lib`) for models and utilities.
...
```bash
golem-cli build                    # Build all components
golem-cli build --component journai-collector  # Build specific component
```
...
```bash
golem-cli deploy --environment local
...
```bash
golem-cli build --preset release  # Build for production
golem-cli deploy --environment local  # Deploy to Golem OSS (needs docker compose up)
golem-cli deploy --preset debug  # Build with debug symbols
```
...
```
golem-journai/
├── components-rust/              # WASM components
...
├── common-rust/                 # Shared code
│   └── common-lib/             # Models and utilities
├── Cargo.toml                   # Workspace configuration
├── golem.yaml                   # Golem Cloud manifest
...
- `components-rust/*/Cargo.toml` - Component-specific dependencies
- `components-rust/*/src/lib.rs` - Main component implementation
- `common-rust/common-lib/src/lib.rs` - Shared models and utilities
- `docker-compose.yml` - Local development setup

## Testing Approach

- Tests are written in the same file or tests/ directory
- Use `#[test]` attribute for test functions
- Run `cargo test` to execute all tests
- Use `cargo test -- --nocapture` for debug output
- Components use integration-style tests with mock data
...
- Components are built as CDylib for WASM compatibility
...
- `JOURNAI_LLM_MODEL` - LLM model for analysis
- `JOURNAI_LLM_ENTRIES_LIMIT` - Entry limit for analysis
...
Edit component's `Cargo.toml` to add Rust dependencies. For WASM dependencies, edit `golem.yaml`.

### Component Structure

Each component follows the same pattern:

- `src/lib.rs` - Main implementation with agent_implementation
- `src/database.rs` - Database operations
- `golem.yaml` - Golem Cloud configuration

## AI Integration

The analyzer component uses LLM for error spike analysis with:

- Structured JSON schema responses
- Context window management
- Mock mode for testing (`JOURNAI_LLM_MOCK=true`)
- Configurable model and limits via environment variables

## API Development

Components expose RESTful APIs through Golem Cloud. Key patterns:

- Use `agent_implementation` macro for component registration
- Implement required traits (Collector, Analyzer, Visualizer)
- Database operations use shared models from common-lib
- Error handling follows APIError patterns

## Common Tasks

### Add new feature to collector

1. Edit `components-rust/journai-collector/src/lib.rs`
2. Add tests in the `#[cfg(test)]` module
3. Run `cargo test --lib`
4. Build with `golem-cli build`

### Modify analyzer prompts

1. Edit constants in `components-rust/journai-analyzer/src/lib.rs`
2. Test with mock mode enabled
3. Verify JSON schema validation

### Update visualizer dashboard

1. Edit `components-rust/journai-visualizer/src/lib.rs`
2. Modify HTML templates and styling
...
```bash
docker-compose up  # For database and golem server
golem-cli deploy --environment local
```
...
```bash
golem-cli deploy --environment cloud
```

### Presets

```bash
golem-cli deploy --preset release
```

## Debugging

- Use `RUST_LOG=debug` for detailed logging
- Check component logs in Golem Cloud dashboard
- Use mock mode (`JOURNAI_LLM_MOCK=true`) for testing without LLM calls
- Database queries can be logged with `DATABASE_QUERY_LOG=true`
